...
Style: Lyrics,DejaVu Sans,78,&H00FFFFFF,&H00FFFFFF,&H6B190C05,&H88000000,-1,0,0,0,100,100,0,0,${cinematic ? 3 : 1},${cinematic ? 18 : 1},0,5,45,45,0,1

[Events]
Format: Layer, Start, End, Style, Name, MarginL, MarginR, MarginV, Effect, Text
`;
  const events = [`Dialogue: 0,0:00:00.00,${clock(duration)},Title,,0,0,0,,{\\pos(540,210)}${literal(title)}`];
  for (const line of lines) {
    const position = animated ? `\\move(540,${y + 24},540,${y},0,350)` : `\\pos(540,${y})`;
    events.push(`Dialogue: 1,${clock(line.start)},${clock(line.end)},Lyrics,,0,0,0,,{${position}\\fad(120,0)}${literal(line.text)}`);
  }
