...
export default function Studio() {
  ...
  const [treatments, setTreatments] = useState<Treatment[]>(["kinetic", "visualizer"]);
  ...
  const refreshJobs = useCallback(async () => {
    const data = await api("/api/studio/batches");
    setJobs(data.jobs);
    return data.jobs as StudioJob[];
  }, []);
  const refreshSchedule = useCallback(async () => {
    const { data, error } = await supabase.from("scheduled_posts").select("id,studio_job_id,platform,run_at,status,error").not("studio_job_id", "is", null).order("run_at", { ascending: false }).limit(100);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const [songRows, accountRows, prefs] = await Promise.all([
          supabase.from("songs").select("id,title,slug").order("created_at", { ascending: false }),
          supabase.from("social_connections").select("platform,username,external_user_id"),
          api("/api/studio/preferences"),
        ]);
        if (songRows.error) throw songRows.error;
        if (accountRows.error) throw accountRows.error;
        if (active) { setSongs(songRows.data ?? []); setConnections(accountRows.data ?? []); setPreferences(prefs); }
        await Promise.all([refreshJobs(), refreshSchedule()]);
  ...
  useEffect(() => {
    ...
    if (songId) (async () => {
      const { data, error } = await supabase.from("song_assets").select("*").eq("song_id", songId).maybeSingle();
      if (!active) return;
      if (error) { setError(error.message); return; }
      if (data) {
        setAsset(data); setDuration(data.duration); setMasterLyrics(data.lyrics);
        const { data: signed } = await supabase.storage.from(ASSET_BUCKET).createSignedUrl(data.audio_path, 3600);
  ...
  const saveAssets = () => action("Saving song…", async () => {
    if (!songId) throw new Error("Choose or add a song first.");
    ...
    const next: StudioAsset = {
      song_id: songId, audio_path: files.audio ? await upload("audio", files.audio, user.id) : asset!.audio_path,
  ...
  const addSong = () => action("Adding song…", async () => {
    if (!title.trim()) throw new Error("Enter a song title.");
    const slug = (title.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-|-$/g, "").slice(0, 50) || "song") + "-" + crypto.randomUUID().slice(0, 6);
    const { data, error } = await supabase.from("songs").insert({ title: title.trim(), slug }).select("id,title,slug").single();
    if (error) throw error;
  ...
  const transcribe = (i: number) => action(`Listening to excerpt ${i + 1}…`, async () => {
    ...
    updateExcerpt(i, { lyrics: data.lyrics }); setMessage("Lyrics transcribed. Listen and correct the words or timestamps before rendering.");
  });
  const createBatch = () => action("Creating drafts…", async () => {
    if (!asset || Object.keys(files).length) throw new Error("Save your song assets before creating a batch.");
  ...
  return <main className="studio-shell">
    ...
    {!loaded && <p role="status">Loading your studio…</p>}
    {step === "create" ? <div className="studio-create">
      <section className="studio-panel"><div className="studio-section-heading"><span className="studio-number">01</span><div><h2>Start with a song</h2><p>Your original audio is all you need.</p></div></div>
        ...
        <label className="studio-upload"><Upload size={26} /><strong>{files.audio?.name || asset?.audio_name || "Drop into your next chapter"}</strong><span>Choose an MP3, WAV, M4A, or original video · up to 50 MB</span><input type="file" accept="audio/*,video/mp4,video/quicktime" disabled={!!busy} onChange={(e) => { const file = e.target.files?.[0]; if (file) { setDuration(0); setFiles((old) => ({ ...old, audio: file })); } }} /></label>
        {audioUrl && <audio ref={audioRef} controls src={audioUrl} preload="metadata" onLoadedMetadata={(e) => { if (Number.isFinite(e.currentTarget.duration)) setDuration(e.currentTarget.duration); }} onTimeUpdate={(e) => { if (previewEnd.current !== null && e.currentTarget.currentTime >= previewEnd.current) { e.currentTarget.pause(); previewEnd.current = null; } }} />}
        ...
        <button className="studio-primary" onClick={saveAssets} disabled={!!busy || !songId}><Check size={16} /> Save song assets</button>
      </section>
      <section className="studio-panel"><div className="studio-section-heading"><span className="studio-number">02</span><div><h2>Find your moments</h2><p>Choose up to three excerpts, 10–45 seconds each.</p></div></div>
        ...
        {excerpts.map((excerpt, i) => <div className="studio-excerpt" key={i}><div className="studio-inline spread"><strong>Moment {i + 1}</strong>{excerpts.length > 1 && <button className="studio-text-button" disabled={!!busy} onClick={() => setExcerpts((old) => old.filter((_, j) => j !== i))}>Remove</button>}</div><div className="studio-time-fields"><label>Start (seconds)<input type="number" min="0" step="0.1" value={excerpt.start} onChange={(e) => updateExcerpt(i, { start: Number(e.target.value) })} /></label><label>Length (seconds)<input type="number" min="10" max="45" value={excerpt.duration} onChange={(e) => updateExcerpt(i, { duration: Number(e.target.value) })} /></label><button onClick={() => preview(excerpt)} disabled={!audioUrl}><Headphones size={15} /> Listen</button></div><label>Lyrics for this moment<textarea dir="auto" rows={4} value={excerpt.lyrics} onChange={(e) => updateExcerpt(i, { lyrics: e.target.value })} placeholder={"[0:00.000] Your first lyric\n[0:04.500] The next line"} /></label><div className="studio-inline"><button onClick={() => transcribe(i)} disabled={!!busy || !asset}><Sparkles size={14} /> Auto-transcribe</button><span className="studio-hint">Uses your transcription service. Review sung words and timing.</span></div></div>)}
        <p className="studio-hint">Timestamps start at zero for each excerpt. Without timestamps, lines are spaced evenly. Paste the exact lyrics to correct the transcript.</p>
